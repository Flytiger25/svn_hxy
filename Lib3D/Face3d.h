...
public:
	std::vector<double> m_Attibuts; //Atributes
	int m_selectedflag;
public:
	...
	void SetVertexIndex(int index1, int index2, int index3)
	{
		m_vertexindex[0] = index1;
		m_vertexindex[1] = index2;
		m_vertexindex[2] = index3;
	}
	...
	int GetMatrial(void)
	{
		return m_IndexMaterial;
	}

	void SetMatrial(unsigned int matrial)
	{
		m_IndexMaterial = matrial;
	}
	...
	int IndexFrom1(CVertex3d* pVertex);

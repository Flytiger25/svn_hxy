    ...
    std::vector<Handle(Geom_BSplineCurve)> guideCurves, Handle(Geom_BSplineSurface)& guidedGordon)
{
    ...
    //TColStd_Array1OfReal uKnotsTCol = originSurf->UKnots();
    //TColStd_Array1OfInteger uMultsTCol = originSurf->UMultiplicities();
    //TColStd_Array1OfReal vKnotsTCol = originSurf->VKnots();
    //TColStd_Array1OfInteger vMultsTCol = originSurf->VMultiplicities();
    ...
    for (int i = 1; i <= knotsU.Size() - 1; ++i)
    {
        for (int j = 1; j <= knotsV.Size() - 1; ++j)
        {
            ...
            for (size_t k = 0; k < pntParams.size(); ++k)
            {
                if (InterPolateTool::isLessThan(pntParams[k].X(), umax, 0.01) && InterPolateTool::isGreaterThan(pntParams[k].X(), umin, 0.01)
                    && InterPolateTool::isLessThan(pntParams[k].Y(), vmax, 0.01) && InterPolateTool::isGreaterThan(pntParams[k].Y(), vmin, 0.01))
                {
                    newOffsets.push_back(offsets[k]);
                    newPntParams.push_back(pntParams[k]);
                    isInDataRegion = true;
            ...
            if (!isInDataRegion)
            {
                newOffsets.push_back(gp_Pnt(0.0, 0.0, 0.0));
                double u = (umin + umax) / 2.0;
                double v = (vmin + vmax) / 2.0;
                newPntParams.push_back(gp_Pnt2d(u, v));
            }
    ...
    // 拟合出偏移曲面
    Handle(Geom_BSplineSurface) offsetSurf;

    /*
    // occ
    TColgp_Array2OfPnt offsetPoints(1, 10, 1, guideCurves.size());
    int t = 0;
    for (int i = 1; i <= 10; i++)
    {
        for (int j = 1; j <= guideCurves.size(); j++)
        {
            offsetPoints(i, j) = offsets[t];
            t++;
        }
    }
    //TColgp_Array2OfPnt offsetPoints = InterPolateTool::vectorToOCCMatrix(offsets, v_direct_poles_num);
    GeomAPI_PointsToBSplineSurface interpolateSurf;
    interpolateSurf.Init(offsetPoints, 3, 3);
    if (interpolateSurf.IsDone())
    {
        offsetSurf = interpolateSurf.Surface();
        TopoDS_Face face = BRepBuilderAPI_MakeFace(offsetSurf, Precision::Confusion());
        STEPControl_Writer stepWriter;
        stepWriter.Transfer(face, STEPControl_AsIs);
        std::string filePath = "D:\\work\\data\\interpolate\\occInterpolate.step";
    ...
    */
    ...
    TColStd_Array1OfReal uKnotsTCol = knotsU;
    TColStd_Array1OfInteger uMultsTCol = multiplicitiesU;
    TColStd_Array1OfReal vKnotsTCol = knotsV;
    TColStd_Array1OfInteger vMultsTCol = multiplicitiesV;
    ...
    offsetSurf = InterPolateTool::Interpolate(newOffsets, newPntParams, uKnots, vKnots, uMults, vMults, 3, 3);
    ...
    std::string filePath = "D:\\work\\data\\offsetSurf\\offsetSurf.step";
    IFSelect_ReturnStatus status = stepWriter.Write(filePath.c_str());
    ...
    {
        offsetSurf->InsertUKnot(UKnots1(i), UMults1(i), 1.e-5, false);
    }
    for (int i = 1; i <= NbVKnot1; i++)
    {
        offsetSurf->InsertVKnot(VKnots1(i), VMults1(i), 1.e-5, false);
    }
    ...
    {
        originSurf->InsertUKnot(UKnots2(i), UMults2(i), 1.e-5, false);
    }
    for (int i = 1; i <= NbVKnot2; i++)
    {
        originSurf->InsertVKnot(VKnots2(i), VMults2(i), 1.e-5, false);
    }
...
// 不拟合偏移曲面，在数据点处加上偏移量
void GuideGordon::GuideGordonSurf2(Handle(Geom_Surface) originalGordon,
    std::vector<Handle(Geom_BSplineCurve)> guideCurves, Handle(Geom_BSplineSurface)& guidedGordon)
{
    Handle(Geom_BSplineSurface) originSurf = Handle(Geom_BSplineSurface)::DownCast(originalGordon);
...
// 12.9 按照论文中实现
void GuideGordon::GuideGordonSurf3(Handle(Geom_Surface) originalGordon, std::vector<double> uIsoparamParams, std::vector<double> vIsoparamParams,
    std::vector<Handle(Geom_BSplineCurve)> guideCurves, Handle(Geom_BSplineSurface)& guidedGordon)
{
    Handle(Geom_BSplineSurface) originSurf = Handle(Geom_BSplineSurface)::DownCast(originalGordon);
    UniformSurface(originSurf);
    ...
    */
    ...
    int uDegree = originSurf->UDegree();
    int vDegree = originSurf->VDegree();
    int numCtrlPtsU = originSurf->NbUPoles();
    int numCtrlPtsV = originSurf->NbVPoles();
    ...
    {
        boundaryIndices.push_back(i * numCtrlPtsV);            // 左边界
        boundaryIndices.push_back(i * numCtrlPtsV + numCtrlPtsV - 1); // 右边界
    }
    for (int j = 0; j < numCtrlPtsV; ++j)
    {
        boundaryIndices.push_back(j);                         // 下边界
        boundaryIndices.push_back((numCtrlPtsU - 1) * numCtrlPtsV + j); // 上边界
    }

    // 等参线参数（这里暂时设为空，可以根据需求加入）
    std::vector<double> uIsoparamParams; // u方向等参线参数（如必要）
    std::vector<double> vIsoparamParams; // v方向等参线参数（如必要）


    TColgp_Array2OfPnt poles = FitConstrainedBSplineSurface(
        newOffsets, newPntParams, uDegree, vDegree,
        stdUKnots, stdVKnots, numCtrlPtsU, numCtrlPtsV,
        boundaryIndices, uIsoparamParams, vIsoparamParams);


    Handle(Geom_BSplineSurface) offsetSurf = new Geom_BSplineSurface(
        poles,
        uKnots, vKnots,
        uMults, vMults,
        uDegree, vDegree);
    ...
    guidedGordon = new Geom_BSplineSurface(resPole, uKnots,
...
/*
void GuideGordon::ComputeBSplineBasis(int degree, const std::vector<double>& knots, double param, std::vector<double>& basis)
{
    ...
    for (int k = 1; k <= degree; ++k)
    {
        for (int i = 0; i < n - k; ++i)
        {
            double left = (param - knots[i]) / (knots[i + k] - knots[i]);
            double right = (knots[i + k + 1] - param) / (knots[i + k + 1] - knots[i + 1]);

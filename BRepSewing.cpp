...
#include <iostream>
#include <TopoDS_Edge.hxx>
#include <BRepTools.hxx> // 如果需要获取 Edge 信息
#include <BRep_CurveOnSurface.hxx>

//添加部分3340-3365
IMPLEMENT_STANDARD_RTTIEXT(BRepSewing, Standard_Transient)
...
//purpose  : 
//=======================================================================
Handle(Geom2d_Curve) BRepSewing::SameRange(const Handle(Geom2d_Curve)& CurvePtr,
...
TopoDS_Face BRepSewing::WhichFace(const TopoDS_Edge& theEdg, const Standard_Integer index) const
{
...
Standard_Boolean BRepSewing::IsUClosedSurface(const Handle(Geom_Surface)& surf,
...
Standard_Boolean BRepSewing::IsVClosedSurface(const Handle(Geom_Surface)& surf,
	const TopoDS_Shape& theEdge,
	const TopLoc_Location& theloc) const
{
	Handle(Geom_Surface) tmpsurf = surf;
...
void BRepSewing::SameParameter(const TopoDS_Edge& edge) const
{
	try {
...
TopoDS_Edge BRepSewing::SameParameterEdge(const TopoDS_Shape& edge,
...
TopoDS_Edge BRepSewing::SameParameterEdge(const TopoDS_Edge& edgeFirst,
...
void BRepSewing::EvaluateAngulars(TopTools_SequenceOfShape& sequenceSec,
...
//=======================================================================
void BRepSewing::EvaluateDistances(TopTools_SequenceOfShape& sequenceSec,
	TColStd_Array1OfBoolean& secForward,
...
//修改代码
void BRepSewing::EvaluateEdgeDistancesModified(const TopoDS_Edge& edge1, const TopoDS_Edge& edge2,
	Standard_Real& maxOfMinDistances, Standard_Real& minOfMinDistances)
{
	...
	c3d1 = BRep_Tool::Curve(edge1, loc1, first1, last1);
	c3d2 = BRep_Tool::Curve(edge2, loc2, first2, last2);

	if (c3d1.IsNull() || c3d2.IsNull()) {
		std::cerr << "Error: One of the curves is null." << std::endl;
		return; // 如果任一曲线为空，则退出
	}
	...
	for (Standard_Integer i = 1; i <= npt; ++i) {
		...
		pts1(i) = c3d1->Value(T1);
		pts2(i) = c3d2->Value(T2);

		//// 输出每个离散点的坐标
		//std::cout << "Point on edge1 (" << i << "): ("
		//    << pts1(i).X() << ", " << pts1(i).Y() << ", " << pts1(i).Z() << ")" << std::endl;
		//std::cout << "Point on edge2 (" << i << "): ("
		//    << pts2(i).X() << ", " << pts2(i).Y() << ", " << pts2(i).Z() << ")" << std::endl;
	}
	...
	for (Standard_Integer i = 1; i <= npt; ++i) {
		...
		for (Standard_Integer j = 1; j <= npt; ++j) {
			Standard_Real dist = pts1(i).Distance(pts2(j));
	...
	for (Standard_Integer j = 1; j <= npt; ++j) {
		Standard_Real minDist = Precision::Infinite();
		for (Standard_Integer i = 1; i <= npt; ++i) {
			Standard_Real dist = pts2(j).Distance(pts1(i));
			minDist = std::min(minDist, dist);
		}
		minDistances.push_back(minDist); // 存储最小距离
		// 输出每个点的最小距离
	...
	maxOfMinDistances = *std::max_element(minDistances.begin(), minDistances.end());
	minOfMinDistances = *std::min_element(minDistances.begin(), minDistances.end());
}
...
Standard_Boolean BRepSewing::IsMergedClosed(const TopoDS_Edge& Edge1,
...
void BRepSewing::AnalysisNearestEdges(const TopTools_SequenceOfShape& sequenceSec,
...
Standard_Boolean BRepSewing::FindCandidates(TopTools_SequenceOfShape& seqSections,
...
BRepSewing::BRepSewing(const Standard_Real tolerance,
...
void BRepSewing::Init(const Standard_Real tolerance,
	const Standard_Boolean optionSewing,
	const Standard_Boolean optionAnalysis,
	const Standard_Boolean optionCutting,
	const Standard_Boolean optionNonmanifold)
...
void BRepSewing::Load(const TopoDS_Shape& theShape)
{
	myReShape->Clear();
...
void BRepSewing::Add(const TopoDS_Shape& aShape)
{
...
void BRepSewing::Perform(const Message_ProgressRange& theProgress)
{
	...
	if (myNbShapes || !myShape.IsNull())
	{
		...
		{
			std::cout << "Bound faces extent: " << myBoundFaces.Extent() << std::endl;
...
const TopoDS_Shape& BRepSewing::SewedShape() const
{
	return mySewedShape;
}
...
Standard_Integer BRepSewing::NbFreeEdges() const
{
...
const TopoDS_Edge& BRepSewing::FreeEdge(const Standard_Integer index) const
{
...
Standard_Integer BRepSewing::NbMultipleEdges() const
{
...
const TopoDS_Edge& BRepSewing::MultipleEdge(const Standard_Integer index) const
{
...
Standard_Integer BRepSewing::NbContigousEdges() const
{
...
const TopoDS_Edge& BRepSewing::ContigousEdge(const Standard_Integer index) const
{
...
const TopTools_ListOfShape& BRepSewing::ContigousEdgeCouple(const Standard_Integer index) const
{
...
Standard_Boolean BRepSewing::IsSectionBound(const TopoDS_Edge& section) const
{
...
const TopoDS_Edge& BRepSewing::SectionToBoundary(const TopoDS_Edge& section) const
{
...
//purpose  : 
//=======================================================================
Standard_Integer BRepSewing::NbDeletedFaces() const
{
...
//purpose  : 
//=======================================================================
const TopoDS_Face& BRepSewing::DeletedFace(const Standard_Integer index) const
{
...
Standard_Integer BRepSewing::NbDegeneratedShapes() const
{
...
const TopoDS_Shape& BRepSewing::DegeneratedShape(const Standard_Integer index) const
{
...
Standard_Boolean BRepSewing::IsDegenerated(const TopoDS_Shape& aShape) const
{
...
Standard_Boolean BRepSewing::IsModified(const TopoDS_Shape& aShape) const
{
...
const TopoDS_Shape& BRepSewing::Modified(const TopoDS_Shape& aShape) const
{
...
Standard_Boolean BRepSewing::IsModifiedSubShape(const TopoDS_Shape& aShape) const
{
	TopoDS_Shape NewShape = myReShape->Apply(aShape);
	if (!NewShape.IsSame(aShape)) return Standard_True;
	return Standard_False;
...
TopoDS_Shape BRepSewing::ModifiedSubShape(const TopoDS_Shape& aShape) const
{
...
void BRepSewing::FaceAnalysis(const Message_ProgressRange& theProgress)
{
				...
				for (; eiter.More(); eiter.Next()) {
					...
					if (!isSmall) {
						...
						if (c3d.IsNull()) {
							...
							// Evaluate curve compactness评估曲线压实度
					...
					else B.Add(nwire, edge); // Old edge kept保留旧边缘
				}

				// Record wire in the new face记录新面中的导线
				if (isWireChanged) {
...
void BRepSewing::FindFreeBoundaries()
{
...
//! function:
//! 合并节点：将 NodeNearestNode 中的节点对合并为新的节点。
//! 更新节点关系：更新新节点与旧节点之间的映射关系。
//! 计算新节点参数：计算新节点的中心点和容差。
//! 更新数据结构：更新包含节点和边的映射关系。
...
{
	// Create new nodes
	// 创建 BRep_Builder 对象 B，用于构建新的几何元素。
	// 初始化 OldNodeNewNode 映射，用于存储旧节点与新节点的对应关系。
	// 初始化 NewNodeOldNodes 映射，用于存储新节点和对应的旧节点列表。
	BRep_Builder B;
	...
	// 遍历 NodeNearestNode 映射中的节点对 oldnode1 和 oldnode2。
	// 以下for循环主要是建立新旧节点的绑定关系
	for (; anIter.More(); anIter.Next()) {
		// Retrieve a pair of nodes to merge检索一对要合并的节点
		const TopoDS_Shape& oldnode1 = anIter.Key();
		const TopoDS_Shape& oldnode2 = anIter.Value();
		// Second node should also be in the map第二个节点也应该在map中
		if (!NodeNearestNode.Contains(oldnode2)) continue;

		// Get new node for old node #1 为旧节点#1获取新节点
		if (OldNodeNewNode.IsBound(oldnode1)) {
			...
			if (OldNodeNewNode.IsBound(oldnode2)) {
				...
				// 如果 newnode1 和 newnode2 是不同的新节点，则需要将它们合并：
				// 获取 newnode1 绑定的旧节点列表 lnode1。
				// 遍历 newnode2 绑定的旧节点列表，将其中的每个旧节点添加到 lnode1 中，并更新它们在 OldNodeNewNode 中的绑定关系，使它们指向 newnode1。
				// 从 NewNodeOldNodes 中移除 newnode2 的键。
	...
	// Stop if no new nodes created如果未创建新节点，则停止
	if (!NewNodeOldNodes.Extent()) return Standard_False;

	// 遍历 NewNodeOldNodes，计算新节点参数并更新数据结构
	...
	for (; anIter1.More(); anIter1.Next()) {
		...
		// Calculate new node center point计算中心点
		...
		for (; itn.More(); itn.Next()) {
			...
			// Iterate on node vertices迭代node绑定的vertices
...
//! 这个函数的主要目的是检查两个顶点 vtx1 和 vtx2 是否可以合并。
//! 它通过检查顶点所在的边和线框是否闭合以及顶点的位置来决定是否合并。
//! 返回值 1 表示顶点可以合并，0 表示不合并， - 1 表示顶点在不同线框中。
...
//! 实现了在容差范围内合并相邻点的功能
//! input:aVertexNode:Vertex与node对应关系的序列，可能是自由边点的序列，可能是浮动边界点的序列；
//! aNodeEdges：顶点与edges的map,传入参数为myNodeSections；
//! aBoundFaces：传入参数为类的myBoundFaces；
//! 
	...
	const Message_ProgressRange& theProgress)
{
	...
	// BRepBuilderAPI_CellFilter 是一个用于空间分割和快速查找的类。它的作用是基于一定的容差，将空间划分成网格单元，以便快速找到处于某个范围内的几何对象。
	// BRepBuilderAPI_CellFilter：用于高效地查找和检索在给定容差范围内的顶点。
	// BRepBuilderAPI_VertexInspector：用于管理顶点信息，并提供查找和检查邻近顶点的功能。
	// TopTools_IndexedDataMapOfShapeShape::Iterator：用于遍历顶点映射容器中的元素。
	...
	//Create map of node -> vertices,建立了NodeVertices这个node与vertices的map
	for (; anIter1.More(); anIter1.Next()) {
	...
	for (Standard_Integer i = 1; i <= nbNodes && aPS.More(); i++, aPS.Next()) {
		...
		// Find near nodes
		// 利用BRepBuilderAPI_CellFilter类和BRepBuilderAPI_VertexInspector进行快速查找，时间复杂度未知，猜想有优化
		...
		//利用aFilter在包围盒内查找，方式未知
		aFilter.Inspect(aPntMin, aPntMax, anInspector);
		if (anInspector.ResInd().IsEmpty()) continue;

		// Retrieve list of edges for the first node检索第一个节点的边列表
		const TopTools_ListOfShape& ledges1 = aNodeEdges(node1);

		// Explore list of near nodes and fill the sequence of glued nodes浏览附近节点的列表，并填写粘贴节点的顺序
		// SeqNodes是node1的最近节点序列，而且按距离进行了排序
		...
		for (; iter1.More(); iter1.Next()) {
			...
			// Retrieve list of edges for the second node检索第二个节点的边列表
			const TopTools_ListOfShape& ledges2 = aNodeEdges(node2);

			// Check merging condition for the pair of nodes检查节点对的合并条件
			Standard_Integer Status = 0, isSameEdge = Standard_False;

			// Explore edges of the first node遍历第一个node的边
			TopTools_ListIteratorOfListOfShape Ie1(ledges1);//迭代器
			for (; Ie1.More() && !Status && !isSameEdge; Ie1.Next()) {
				const TopoDS_Shape& e1 = Ie1.Value();

				// Obtain real vertex from edge从边获得实顶点
				TopoDS_Shape v1 = node1;


				//代码段：确保使用正确的顶点：通过检查 ov1 和 ov2 是否在 aVertexNode 中，并与 node1 进行比较，
				//可以确保 v1 是与 node1 对应的真实顶点，而不仅仅是一个临时的或不准确的顶点表示。
				{ //szv: Use brackets to destroy local variables
				...
				// Create map of faces for e1，el是node1的edge,按理来说，e1一定是个自由边界或者浮动边界，最多只有一个face
				TopTools_MapOfShape Faces1;
				...
				// Explore edges of the second node遍历第二个节点的边
				...
				for (; Ie2.More() && !Status && !isSameEdge; Ie2.Next()) {
					...
					// Explore faces for e2 遍历faces for e2
					...
					if (lfac2.Extent()) {
						...
						for (; itf.More() && !Status && !isSameEdge; itf.Next()) {
							// Check merging conditions for the same face检查同一面的合并条件
							if (Faces1.Contains(itf.Value())) {
								// 这个函数主要检查两个顶点能否合并
								//返回的状态可能是0，-1，1，1是同一条edge?
			...
			// Append near node to the sequence将近节点附加到序列，会按序
		...
		//SeqNodes就是找到的容差范围内接近点序列
		//这段if代码主要是过滤节点，移除 SeqNodes 中与同一条边上的节点过于接近的节点，以避免在后续操作中处理不必要的节点合并。
		if (SeqNodes.Length()) {
			...
			// Bind nearest node if at least one exists绑定最近的节点（如果至少存在一个），插入到NodeNearestNode这个哈希中
			if (SeqNodes.Length())
	...
	// Create new nodes for chained nearest nodes为链接的最近节点创建新节点
...
//! function:
//! 建立node与edges的map
//! 调用GlueVertices()合并容差范围内的接近点，生成新的node
//! 初始化并更新整个类中，newNode与edges的map, newnode与Vertexs的map, 
//! 迄今已经更新完成类的map类型的成员变量：myVertexNode,myNodeSections,myBoundFaces
void BRepSewing::VerticesAssembling(const Message_ProgressRange& theProgress)
{
	//nbVert：自由边顶点与node序列
	//nbVertFree：浮动边界Vertex与Node序列
	...
	if (nbVert || nbVertFree) {
		...
		// Glue vertices在容差范围内合并node-vertices序列
		if (nbVert) {
#ifdef OCCT_DEBUG
	...
	const Handle(BRepTools_ReShape)& aReShape)
{
	...
	//修改源码2024.5.25
	//newShape.AddOldShape(oldShape.TShape());
...
void BRepSewing::Merging(const Standard_Boolean /* firstTime */,
	const Message_ProgressRange& theProgress)
{
	BRep_Builder B;
	...
	for (; anIterB.More() && aPS.More(); anIterB.Next(), aPS.Next()) {
		...
		// If bound was already merged - continue已经被合并，continue
		if (myMergedEdges.Contains(bound)) continue;

		// 针对浮动边界，只合并顶点，不合并edge
		...
		// Check for previous splitting, build replacing wire检查以前的拆分、构建替换wire
		// hasCuttingSections 是否被拆分
		TopoDS_Wire BoundWire;
		Standard_Boolean isPrevSplit = Standard_False;//是否被前置拆分
		...
		if (!isPrevSplit) {
			...
			if (MergedNearestEdges(bound, seqMergedWithBound, seqMergedWithBoundOri)) {
				...
				while (ii <= seqMergedWithBound.Length()) {
					...
					if (!isRejected) {
						...
						if (!isRejected) {
							if (mySectionBound.IsBound(iedge)) {
								// Edge is a section - check bound， edge可能是子序列，检查上层的edge有没有被合并
					...
					// To the next merged edge到下一条合并的边
					if (isRejected) {
						// Remove rejected edge删除被拒绝的边
						seqMergedWithBound.Remove(ii);
						seqMergedWithBoundOri.Remove(ii);
					}
					else {
						// Process accepted edge处理接受的边缘
				...
				if (nbMerged) {
					// Create same parameter edge创建相同的参数边
					...
					// Process actually merged edges处理实际合并的边
		...
		//修改代码20240604  修改的起点
		auto MergedWithBoundNums = MergedWithBound.Extent();
		if (MergedWithBoundNums > 0) {
			TopTools_IndexedDataMapOfShapeShape::Iterator itBounds(MergedWithBound);
			TopoDS_Shape oldShape1 = itBounds.Key();
			TopoDS_Shape newShape1 = itBounds.Value();
			itBounds.Next();
			TopoDS_Shape oldShape2 = itBounds.Key();
			TopoDS_Shape newShape2 = itBounds.Value();

			myOldEdgeNewEdge.Add(oldShape1, newShape1);
			myOldEdgeNewEdge.Add(oldShape2, newShape2);

			if (mySectionBound.IsBound(oldShape1))
				TopoDS_Shape originShape = mySectionBound.Find(oldShape1);//这个就是旧的shape

			TopTools_ListOfShape oldShape;
			oldShape.Append(oldShape1);
			oldShape.Append(oldShape2);
			myNewEdgeOldEdges.Bind(newShape1, oldShape);
			TopoDS_Edge edge1 = TopoDS::Edge(oldShape1);
			TopoDS_Edge edge2 = TopoDS::Edge(oldShape2);
			Standard_Real maxOfMinDistances, minOfMinDistances;
			EvaluateEdgeDistancesModified(edge1, edge2, maxOfMinDistances, minOfMinDistances);//这个就是具体的hausdorff距离

			//// 输出计算结果到控制台
			////std::cout << "Edge Pair:" << std::endl;
			////std::cout << "Edge 1: " << EdgeToString(edge1) << std::endl;
			////std::cout << "Edge 2: " << EdgeToString(edge2) << std::endl;
			//std::cout << "Maximum of Minimum Distances: " << maxOfMinDistances << std::endl;
			//std::cout << "Minimum of Minimum Distances: " << minOfMinDistances << std::endl;

			myEdgePairs.emplace_back(edge1, edge2, maxOfMinDistances);
		}
		//修改结束
		...
		//modify 20240806
		auto MergedWithSectionNums = MergedWithSections.Extent();
		if (MergedWithSectionNums > 0)
		{
			TopTools_IndexedDataMapOfShapeShape::Iterator itBounds(MergedWithSections);
			TopoDS_Shape oldShape1 = itBounds.Key();
			TopoDS_Shape newShape1 = itBounds.Value();
			itBounds.Next();
			TopoDS_Shape oldShape2 = itBounds.Key();
		...
		if (isSplitted) {
			...
			for (; anItm.More(); anItm.Next()) {
				...
				//TshapeMaps[newedge.TShape()] = oldedge.TShape();
				ReplaceEdge(myReShape->Apply(oldedge), newedge, myReShape);
				myMergedEdges.Add(oldedge);
...
//! 找到与给定边（edge）相邻并且可以合并的边。
//! 该函数会检查两端节点并找到与这些节点相连的所有可能的连续边，然后返回这些边及其方向。
Standard_Boolean BRepSewing::MergedNearestEdges(const TopoDS_Shape& edge,
	TopTools_SequenceOfShape& SeqMergedEdge,
	TColStd_SequenceOfBoolean& SeqMergedOri)
{
	// Retrieve edge nodes 获取边顶点
	TopoDS_Vertex no1, no2;
	...
	// Find all possible contiguous edges 查找所有可能的连续边
...
//! function：
//! BRepBuilderAPI_Sewing::Cutting 函数的主要功能是对边界进行切割处理。
//! 通过查找并切割在一定容差范围内相近的顶点来更新边界信息。
void BRepSewing::Cutting(const Message_ProgressRange& theProgress)
{
	Standard_Integer i, nbVertices = myVertexNode.Extent();
	if (!nbVertices) return;

	// Create a box tree with vertices创建vertices的包围盒树
	// 创建一个包围盒树 aTree 和填充器 aTreeFiller。
	// 遍历所有顶点，创建包围盒并填充到树中。
	// 包围盒根据顶点位置和容差生成，并进行适当扩展。

	// BRepBuilderAPI_BndBoxTree：
	// 包围盒树是一种空间分区结构，用于加速几何体的查询和操作，比如碰撞检测、最近邻搜索等。
	// 
	// NCollection_UBTreeFiller<Standard_Integer, Bnd_Box> ：
	// 是 Open CASCADE 中用于填充包围盒树（Bounding Box Tree）的模板类。它的作用是将节点添加到包围盒树中，从而构建出用于快速空间查询和操作的树结构。
	// 
	// BRepBuilderAPI_BndBoxTreeSelector：
	// BRepBuilderAPI_BndBoxTreeSelector 的主要作用是提供一种机制，根据指定的选择标准，
	// 从包围盒树中选择符合条件的节点。
	// 它通常与 BRepBuilderAPI_BndBoxTree 类一起使用，用于高效地在几何体的包围盒树中进行搜索和选择。
	...
	//TopLoc_Location用于表示几何变换。它封装了一个或多个变换（如平移、旋转、缩放等）的组合，并用于描述几何体的位置和方向。
	Handle(Geom_Curve) c3d;
	TopLoc_Location loc;
	Standard_Real first, last;

	// Iterate on all boundaries在所有边界上迭代
	Standard_Integer nbBounds = myBoundFaces.Extent();//Extent()函数是获取个数
	Message_ProgressScope aPS(theProgress, "Cutting bounds", nbBounds);
	TopTools_IndexedDataMapOfShapeListOfShape::Iterator anIterB(myBoundFaces);
	// 这个for循环迭代所有自由边，实现了如下功能：
	for (; anIterB.More() && aPS.More(); anIterB.Next(), aPS.Next()) {
		const TopoDS_Edge& bound = TopoDS::Edge(anIterB.Key());

		// Do not cut floating edges不要切割浮动边缘，如果是浮动边界，continue
		if (!anIterB.Value().Extent()) continue;

		// Obtain bound curve获取绑定曲线
		c3d = BRep_Tool::Curve(bound, loc, first, last);
		if (c3d.IsNull()) continue;

		// 若有TopLoc_Location， 曲线变换
		if (!loc.IsIdentity()) {
			c3d = Handle(Geom_Curve)::DownCast(c3d->Copy());
			c3d->Transform(loc.Transformation());
		}

		// Create cutting sections创建切割Sections
		TopTools_ListOfShape listSections;

		//局部代码段
		{ //szv: Use brackets to destroy local variables
			// Obtain candidate vertices获取候选顶点，生成候选节点set
			TopoDS_Vertex V1, V2;
			TopTools_IndexedMapOfShape CandidateVertices;
			//局部代码段
			{ //szv: Use brackets to destroy local variables
			// Create bounding box around curve创建曲线包围盒
				Bnd_Box aGlobalBox;
				GeomAdaptor_Curve adptC(c3d, first, last);
				BndLib_Add3dCurve::Add(adptC, myTolerance, aGlobalBox);

				// Sort vertices to find candidates对顶点排序以查找候选顶点
				// 这里是用包围盒树进行查找的
				...
				// Retrieve bound nodes获取边界边的两个顶点
				...
				// Fill map of candidate vertices填充候选顶点map
			...
			// 获取候选顶点的数量。如果没有候选顶点，跳过当前边界，继续处理下一个边界。
			Standard_Integer nbCandidates = CandidateVertices.Extent();//候选点数量
			if (!nbCandidates) continue;

			// Project vertices on curve在曲线上投影顶点
			// 定义用于存储投影结果的数组：
				// arrPara 用于存储每个候选顶点在曲线上的参数值。
				// arrDist 用于存储每个候选顶点到曲线的距离。
				// arrPnt 用于存储每个候选顶点的原始位置。
				// arrProj 用于存储每个候选顶点在曲线上的投影点。
			...
			ProjectPointsOnCurve(arrPnt, c3d, first, last, arrDist, arrPara, arrProj, Standard_True);//投影计算函数

			// Create cutting nodes创建切割节点
			// 定义序列 seqNode 和 seqPara 用于存储切割节点和对应的参数值
			TopTools_SequenceOfShape seqNode;
			TColStd_SequenceOfReal seqPara;

			// 调用 CreateCuttingNodes 函数，根据投影结果创建切割节点，并将结果存储在 seqNode 和 seqPara 中。
			CreateCuttingNodes(CandidateVertices, bound,
				V1, V2, arrDist, arrPara, arrProj, seqNode, seqPara);

			if (!seqPara.Length()) continue;
			// Create cutting sections创建cutting section
		...
		if (listSections.Extent() > 1) {
			...
			//                     myBoundSections
			// 
			// 这里新增myBoundSections，mySectionBound，修改myNodeSections
			...
			for (; its.More(); its.Next()) {
				...
				for (TopoDS_Iterator itv(section); itv.More(); itv.Next()) {
					...
					// Convert vertex to node 将vertex转化为node
...
void BRepSewing::GetFreeWires(TopTools_IndexedMapOfShape& MapFreeEdges,
...
void BRepSewing::EdgeProcessing(const Message_ProgressRange& theProgress)
{
...
void BRepSewing::EdgeRegularity(const Message_ProgressRange& theProgress)
{
...
void BRepSewing::CreateSewedShape()
{
	// ---------------------
...
void BRepSewing::CreateOutputInformations()
{
...
void BRepSewing::ProjectPointsOnCurve(const TColgp_Array1OfPnt& arrPnt,
...
void BRepSewing::CreateCuttingNodes(const TopTools_IndexedMapOfShape& MapVert,
...
void BRepSewing::CreateSections(const TopoDS_Shape& section,
...
void BRepSewing::SameParameterShape()
{
...
//purpose  : 
//=======================================================================
const Handle(BRepTools_ReShape)& BRepSewing::GetContext() const
{
	return myReShape;
}
...
//purpose  : 
//=======================================================================
void BRepSewing::SetContext(const Handle(BRepTools_ReShape)& theContext)
{
...
inline  void BRepSewing::SetMaxTolerance(const Standard_Real theMaxToler)
{
...
inline  Standard_Real BRepSewing::MaxTolerance() const
{
...
inline  Standard_Real BRepSewing::Tolerance() const
{
	return myTolerance;
}
...
inline  void BRepSewing::SetTolerance(const Standard_Real theToler)
{
...
inline  void BRepSewing::SetMinTolerance(const Standard_Real theMinToler)
{
...
inline  Standard_Real BRepSewing::MinTolerance() const
{
...
inline  void BRepSewing::SetFaceMode(const Standard_Boolean theFaceMode)
{
...
inline  Standard_Boolean BRepSewing::FaceMode() const
{
	return myFaceMode;
}
...
inline  void BRepSewing::SetFloatingEdgesMode(const Standard_Boolean theFloatingEdgesMode)
{
...
inline  Standard_Boolean BRepSewing::FloatingEdgesMode() const
{
...
inline  Standard_Boolean BRepSewing::SameParameterMode() const
{
...
inline  void BRepSewing::SetSameParameterMode(const Standard_Boolean SameParameterMode)
{
...
inline  void BRepSewing::SetLocalTolerancesMode(const Standard_Boolean theLocalTolerancesMode)
{
...
inline  Standard_Boolean BRepSewing::LocalTolerancesMode() const
{
...
inline void BRepSewing::SetNonManifoldMode(const Standard_Boolean theNonManifoldMode)
{
...
inline  Standard_Boolean BRepSewing::NonManifoldMode() const
{
